...
class PhasorPlot:
    ...
    def plot(self, x, y):
        ...
        plt.plot(x, y, 'ro')            #draw the data points
        x = 1 / ((self.lifetimes * self.omega) ** 2 + 1)
        y = x * (self.lifetimes * self.omega)
        plt.plot(x, y, '.')             #draw the ns markers on hemicircle
        if self.lifetimeline is not None:       #<<<<<<<<<<<<<<< I HAVE NO FAINT IDEA what this is supposed to do. 
...
class SimFalcon:
    ...
    def showhistogram(self):
        """
        plots the decaycurve on a lin-lin scale to visualize the generated data
        :return:
        """
        hist, bincenters = self._gethistogram()
        plt.scatter(bincenters, hist, s=2)
        plt.xlabel('arrivaltime (ns)')
        plt.ylabel("#")
        
    def showloghistogram(self):
        """
        plots the decaycurve on a log-lin scale to visualize the generated data
        :return:
        """
        hist, bincenters = self._gethistogram()
        plt.scatter(bincenters, hist, s=2)
    ...
    def setphotons(self, photons):
        """
        Set photons, replacing original photons.  <<<<<<<<<<<<<<<@ROLF what do you mean with this?
        :param photons:
        :return:
        """
        self.photons = photons
    ...
    def showphasor(self, lifetimes=np.arange(0, 6, 1), lifetimeline=None):
        """
        >>>>>>>>>>>>>>>>>>>>>>>IF I AM CORRECT this method is not used anywhere!!!!
        It seems to do the same as class PhasorPlot                 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        :return:
        """
...
class SimSample:
    ...
    def __init__(self, lifetimes, fractions, fractionnames=None):
        if fractionnames is None:
            ...
            self.fractionnames = fractionnames.copy()  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ROLF waarom heb je hier overal .copy en in andere classes niet?
    ...
    def generate_photons(self, number_of_photons=1e6, seed=None):
        ...
        npht[-1] = int(number_of_photons - npht[0:-1].sum()) #<<<<<WHAT IS HAPPENING HERE, Rolf??
        print("npht----2: ",npht)                                #<<<<<<<<<<<<<<<<<<<<<< debugging
        ...
        #and then simulate them with rng.exponential(Tau, size) and add them together, right?
...
fractions = [50, 40, 10, 5]
fractionnames = ['Epac bound', 'Epac unbound', 'AF long', 'AF short']
simSample = SimSample(lifetimes, fractions, fractionnames)
print(simSample)

qv = [0.02, 0.425,0, 0] #this is used to calculate the dye brightness when quenched by FRET
simSample.set_quenching(qv)
print(simSample)

fraction = np.linspace(0,100,101) # fraction is in %, i.e. 0-100
...
simFalcon.showloghistogram() #data are based on the last run of the fraction iteration loop
plt.show()
