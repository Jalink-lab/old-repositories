...
for cell in cell_list:
    ...
    im, xedges, yedges, h_ = ax2.hist2d(dist_to_lam, dist_to_his, bins=[xedges, yedges])
    plt.colorbar(h_, ax=ax2)
    xcentres = (xedges[1:]+xedges[:-1])/2
    ycentres = (yedges[1:] + yedges[:-1]) / 2
    ax1.plot(np.nansum(im, axis=0), ycentres)
    ax1.set_ylabel('Distance from Histone Mark (nm)')
    ax1.invert_xaxis()
    ax4.plot(xcentres, np.nansum(im, axis=1))
    ax4.set_xlabel('Distance from Lamina (nm)')
