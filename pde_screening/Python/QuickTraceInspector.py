...
users_base_path = filedialog.askopenfilename(title="Choose a file", filetype=(('tsv files', '*.tsv'),("all files", "*.*")))
...
#labelmapfile = Path(users_base_path.parent.parent,'intensity_data',users_base_path.name[:-8]+'_I_mean_labelmap.tif')
...
autoScroll=False #automatically scrolls through the cells
randomCells=False #only show a few (max_nroi) examples
pickSpecificCell=False #only show (a) specific cell(s)
if pickSpecificCell==True:
    randomCells=False
fixYScale = False
max_nroi=500           #<<<<<<<<<< adjust this one to set how many are plotted maximally per well
scrollTime=0.1        #autoscroll delay, in s
...
for trace_id in selectedCells:
    ...
    mean_init_int = fit.Mean_init_int[trace_id]
    max_in256 = np.max(fit.Mean_init_int)/256
    mean_init_int = mean_init_int/max_in256
