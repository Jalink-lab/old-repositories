% data 07-11-2019 (Chemical)
% 1 datapoint from 2020 with IBMX
clear all; close all; clc
pth = 'E:\2020\02\06\chemical\result';
...
for ct = 1:length(d)
    alldata{ct,1}=d(ct).name(1:3);
    if(alldata{ct,1}(end)=='_'),alldata{ct,1}(end)=[];end
...
fclose(fid);
if length(unique(wellnrs))~=length(wellnrs),warning('used a well nr more than once');end
if length(unique(wellnrs))~=size(alldata,1),warning('did not use well %s',alldata{setdiff(1:size(alldata,1),wellnrs),1});end
clear alldata;
%% show a cloud
% I = [];
...
fig=gcf;
fig.Position=[100 200 700 400];

ax.XTickLabel = strrep(xticklabels,'uM','\muM');
fig.Children(1).TickLabelInterpreter='tex';
fig.Children(1).FontSize=9;
fig.Children(1).FontName = 'Times New Roman';
% xticklabels = strrep(xticklabels,'u','$\mu$');
% ax.XTickLabel = strrep(xticklabels,'_','\textsuperscript');
ax.XTickLabelRotation = -45;

fig.Children(1).Position(1)=0.06;
fig.Children(1).Position(3)=0.84;
saveas(fig,'figure7.emf')