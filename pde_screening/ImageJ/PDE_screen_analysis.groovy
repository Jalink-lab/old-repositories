...
#@ Double(label=="Stability range(ns)",value=0.2, description="Stable lifetime range in first frames, to determine base lifetime") stabilityRange
#@ Boolean(label="Remove last frame",value=true) removeLastFrame
#@ Boolean(label="Forskolin as endpoint?",value=true) forskolinBoolean
#@ Boolean(label="Analyse entire field?",value=false) analyse_entire_field
#@ Double(label="Frame interval (seconds)",value=600, description="Will only be used when 'Analyze entire field' is checked") frameInterval
*/
...
def main() {
	...
	dirTwoComponents.eachFileMatch FILES, ~".*.tif", {//When File is a folder it can iterate over all files in a loop. In the loop the individual files are called 'it'
		...
		def well = it.name[0..2] //first 3 characters
		...
		selectedFitData = allFitData
		if(!analyse_entire_field) {
...
def openFile(fileTwoCP,well) { //match .tif file to the .zip file
	...
	if (analyse_entire_field){
		...
		dirROI.eachFileMatch FILES, ~"PDE_Screen_.*$well.*.zip",{fileROI=it}
		def fileTiff = new File("")
		dirROI.eachFileMatch FILES, ~"PDE_Screen_.*$well.*.tif",{fileTiff=it}
		println "Processing:\n$fileTwoCP\n$fileROI\n$fileTiff\n--------"
		IJ.run("Close All", "");
		def impTiff = IJ.openImage(fileTiff.toString())
		cal = impTiff.getLocalCalibration()
		frameInterval = cal.frameInterval
		println("Setting frame interval to "+frameInterval+" sec / frame")
		impTiff.close()
		def imp = IJ.openImage(fileTwoCP.toString())
		imp.show()
		rm = RoiManager.getRoiManager()
...
def FancyTable getLifetimesFromROI() {
	...
	for (int fr=0;fr<nFr;fr++){
		 intensity.setValue("Time(s)",fr,fr*frameInterval) //column, row, value
		 lifetime.setValue("Time(s)",fr,fr*frameInterval) //column, row, value
...
def FancyTable getindexFitRange(FancyTable ft){
	...
	for (int i=0;i<diffmeanTau.size();i++){ //find maximum in first 200 seconds
		...
		if (meanTau[i]>PPP[1] && i<(150/frameInterval)){ //only 150 seconds
			PPP[0]=i;
			PPP[1]=meanTau[i]
		}
	}
	...
	if(forskolinBoolean){
		...
		for (int i = (int)FKP[0];i>(int)(FKP[0]-20);i--){
			if (meanTau[i]>meanTau[i+1]){
				FKP[0]=(double) i
				break
			}
		}
	} else {
		FKP[0] = diffmeanTau.size()-1
	}
	FKP[1]=meanTau[(int)FKP[0]]
	meanplot = new Plot("Debug - Mean plot","time(s)","lifetime(ns)")
	meanplot.add("line",ft.getTimeAxis(),meanTau)
	double[] xVal = [PPP[0]*frameInterval,FKP[0]*frameInterval]
	double[] yVal = [PPP[1],meanTau[(int)(FKP[0])]]
	...
	int[] indexFitRange = [PPP[0],FKP[0]] //two points
