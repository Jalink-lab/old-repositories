#@ File (label = "Tilescan image file", style = "file") tilescan_image
#@ File (label = "OME-XML file containing x-y positions", style = "extensions:xml") xml_file_path
#@ File (label = "ROIs file", style = "file", style = "extensions:zip") ROIs_file
...
//output_dir = "C:\\Users\\b.vd.broek\\surfdrive\\SP8 cell positions";
...
Plot.create("Positions graph", "X (m)", "Y (m)");
...
Plot.setStyle(1, "#667788,#ddeeff,1.0,+");
...
selectWindow("Log");
saveAs("text",output_dir+File.separator+"selected_positions.txt");
if(File.exists(output_dir+File.separator+"selected_positions.rgn")) File.delete(output_dir+File.separator+"selected_positions.rgn");
File.rename(output_dir+File.separator+"selected_positions.txt",output_dir+File.separator+"selected_positions.rgn");

print(Cell_pos_X.length+" positions saved in "+output_dir+File.separator+"selected_positions.rgn");
